...
class SlamMapper:
    ...
    def match(self, pts, seed):
        ...
        stages = ((self.args.search_lin, 0.08,
                   self.args.search_ang, math.radians(2.5)),
    ...
    def process_scan(self, angles_deg, ranges_m):
        ...
        new_pose, score = self.match(pts, pred)
        ...
        if score < self.args.min_score or jump > self.args.max_jump:
            self.n_rejected += 1
            self.velocity[:] = 0.0
            return False
...
def build_parser():
    ...
    s.add_argument('--min-score', type=float, default=0.18,
                   help='reject matches below this mean field score')
    ...
    try:
        while True:
            ...
            if not args.quiet and time.time() - last_log > 2.0:
                ...
                print('[slam] scans=%d kf=%d rej=%d  pose=(%.2f, %.2f, %.1f deg)'
