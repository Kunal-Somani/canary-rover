...
def main(argv=None):
    ...
    # Heading compass, fixed in a corner of the window (figure coordinates,
    # so it doesn't pan/zoom with the map). "N" marks 0 deg -- the direction
    # the RPLiDAR's zero-angle arrow pointed when the mapper started, since
    # there's no magnetometer to tie it to true north.
    compass_ax = fig.add_axes([0.83, 0.83, 0.14, 0.14])
    compass_ax.set_xlim(-1.35, 1.35)
    compass_ax.set_ylim(-1.35, 1.35)
    compass_ax.set_aspect('equal')
    compass_ax.axis('off')
    compass_ax.add_patch(Circle((0, 0), 1.0, facecolor='white', edgecolor='#d0d0d4',
                                lw=1.0, alpha=0.85, zorder=1))
    for deg, label in ((0, 'N'), (90, 'W'), (180, 'S'), (270, 'E')):
        rad = np.deg2rad(deg)
        compass_ax.text(-np.sin(rad) * 1.18, np.cos(rad) * 1.18, label,
                        ha='center', va='center', fontsize=8, fontweight='bold',
                        color='#5b5b66')
    (compass_needle,) = compass_ax.plot([0, 0], [0, 0.8], color='#f76b15', lw=2.4,
                                        solid_capstyle='round', zorder=3)
    compass_ax.plot(0, 0, marker='o', markersize=3.5, color='#5b5b66', zorder=3)
    compass_label = compass_ax.text(0, -1.32, '', ha='center', va='top', fontsize=7,
                                    color='#5b5b66')
    ...
    def update(_frame):
        ...
        if pose is not None:
            ...
            # th=0 is "up" on the compass (the arrow's start direction); turning
            # left (th increasing, the usual CCW convention) swings the needle
            # left too, same as rotating a vector that starts pointing up.
            compass_needle.set_data([0, -np.sin(th) * 0.8], [0, np.cos(th) * 0.8])
            compass_label.set_text('%+.0f deg' % np.degrees(th))
        ...
        return im, traj_line, scan, heading, robot, status, compass_needle, compass_label
