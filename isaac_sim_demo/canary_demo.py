...
def setup_lights(stage):
    ...
    sun.CreateAngleAttr(0.53) # Softens shadows naturally
...
def setup_ros2_bridge(stage):
    import omni.graph.core as og
    import omni.kit.commands

    print("\n[ROS2] Building ActionGraph...")

    lidar_path = "/World/base_link/lidar"

    try:
        omni.kit.commands.execute(
            'IsaacSensorCreateRtxLidar',
            path="lidar",
            parent="/World/base_link",
            config="Example_Rotary",   
        )
        lidar_prim = stage.GetPrimAtPath(lidar_path)
        if lidar_prim.IsValid():
            xf  = UsdGeom.Xformable(lidar_prim)
            ops = {op.GetOpName(): op for op in xf.GetOrderedXformOps()}
            if "xformOp:translate" in ops:
                ops["xformOp:translate"].Set(Gf.Vec3d(0.0, 0.0, 0.4))
            else:
                xf.AddTranslateOp().Set(Gf.Vec3d(0.0, 0.0, 0.4))
            print("[ROS2] ✓ RTX Lidar created at /World/base_link/lidar")
        else:
            print("[ROS2] ✗ Lidar prim not valid after creation")
            lidar_path = None
    except Exception as e:
        print(f"[ROS2] ✗ IsaacSensorCreateRtxLidar: {e}")
        lidar_path = None
    ...
    try:
        ...
        connect = [
            ...
            ("Time.outputs:simulationTime", "ClockPub.inputs:timeStamp"),
            ("Time.outputs:simulationTime", "TFPub.inputs:timeStamp"),
        ...
        if lidar_path:
            create_nodes += [
                ("LidarPC",  "omni.isaac.sensor.IsaacComputeRTXLidarPointCloud"),
                ("ScanPub",  "isaacsim.ros2.bridge.ROS2PublishLaserScan"),
            ]
            connect += [
                ("Tick.outputs:tick",              "LidarPC.inputs:execIn"),
                ("Time.outputs:simulationTime",    "ScanPub.inputs:timeStamp"),
                ("LidarPC.outputs:execOut",        "ScanPub.inputs:execIn"),
                ("LidarPC.outputs:azimuthRange",   "ScanPub.inputs:azimuthRange"),
                ("LidarPC.outputs:depthRange",     "ScanPub.inputs:depthRange"),
                ("LidarPC.outputs:horizontalFov",  "ScanPub.inputs:horizontalFov"),
                ("LidarPC.outputs:horizontalResolution", "ScanPub.inputs:horizontalResolution"),
                ("LidarPC.outputs:intensitiesData","ScanPub.inputs:intensitiesData"),
                ("LidarPC.outputs:linearDepthData","ScanPub.inputs:linearDepthData"),
                ("LidarPC.outputs:numCols",        "ScanPub.inputs:numCols"),
                ("LidarPC.outputs:numRows",        "ScanPub.inputs:numRows"),
                ("LidarPC.outputs:rotationRate",   "ScanPub.inputs:rotationRate"),
            ]
            set_values += [
                ("ScanPub.inputs:topicName", "/scan"),
                ("ScanPub.inputs:frameId",   "lidar"),
            ]
        ...
        og.Controller.set(og.Controller.attribute("/World/ROS2_Graph/TFPub.inputs:parentPrim"), "/World/base_link")
        if lidar_path:
            og.Controller.set(og.Controller.attribute("/World/ROS2_Graph/TFPub.inputs:targetPrims"), [lidar_path])
            og.Controller.set(og.Controller.attribute("/World/ROS2_Graph/LidarPC.inputs:lidarPrim"), [lidar_path])

        for attr_name in ["inputs:robotPrim", "inputs:chassisPrim"]:
            try:
                og.Controller.set(og.Controller.attribute(f"/World/ROS2_Graph/OdomPub.{attr_name}"), ["/World/base_link"])
                print(f"[ROS2] ✓ OdomPub wired via {attr_name}")
                break
            except Exception:
                continue

        print("[ROS2] ✓ Graph active — topics: /clock  /tf  /odom" + ("  /scan" if lidar_path else ""))
        print()

    except Exception as e:
        print(f"[ROS2] ✗ ActionGraph Error: {e}\n")
...
def main():
    ...
    print("  IMU (60 Hz) | RTX LiDAR | ROS2: /clock /tf /odom /scan")
    print("="*72 + "\n")
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing() and not bridge_initialized and world.current_time_step_index > 60:
            setup_ros2_bridge(stage)
        ...
        if world.is_playing():
            ...
            if world.current_time_step_index % 60 == 0 and bridge_initialized:
